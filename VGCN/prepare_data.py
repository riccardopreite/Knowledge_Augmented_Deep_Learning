...
if cfg_ds=='sst':
    ...
    train = open("data/IMDb/train.txt", encoding='windows-1252').readlines()
    valid = open("data/IMDb/dev.txt", encoding='windows-1252').readlines()
    test = open("data/IMDb/test.txt", encoding='windows-1252').readlines()
    random.shuffle(train)
    random.shuffle(valid)
    random.shuffle(test)
    train_size=len(train)
    valid_size=len(valid)
    test_size=len(test)
    print('IMDb, train_szie:%d, valid_size:%d, test_size:%d'%(train_size,valid_size,test_size))
    trainset = {}
    validset = {}
    testset = {}
    for data, dataset in [(train, trainset), (valid, validset), (test, testset)]:
        ...
        for line in data:
            splitted = line.split("\t")
            label.append(int(splitted[0]))
            all_text.append(splitted[1])
        dataset["label"] = label
        dataset["data"] = all_text
