...
class RobertaModel(FairseqLanguageModel):
    ...
    def register_ke_head(self, name, num_relations=None, **kwargs):
        ...
        #    self.args.encoder_embed_dim,
            self.args,
            gamma=8, #self.args.gamma,
            nrelation=28418 #self.args.nrelation
        )
...
class RobertaKnowledgeEmbeddingHead(nn.Module):
    ...
    def __init__(self, args, gamma=0, nrelation=0):
        super().__init__()
        if gamma == 0:
            gamma = args.gamma if args.gamma is not None else 8
        if nrelation == 0:
            nrelation = args.nrelation if args.nrelation is not None else 28418 #args.nrelation
        ...
        self.score_func = model_func['TransE']#model_func[args.ke_model]
