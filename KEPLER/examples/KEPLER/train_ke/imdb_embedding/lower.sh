...
#ROBERTA_PATH="mlm_checkpointsnomaxtokens/checkpoint_best.pt" #Path to the original roberta model
ROBERTA_PATH="mlm.pt"
#ROBERTA_PATH="ke.pt"
CHECKPOINT_PATH="lower_ke_attempt" #Directory to store the checkpoints
...
fairseq-train $DATA_DIR --arch roberta_base --num-workers 1 --fix-batches-to-gpus --sample-break-mode eos --separator-token 2\
        --KEdata $KE_DATA \
        ...
        --ddp-backend c10d --log-format tqdm --reset-optimizer --distributed-world-size ${DIST_SIZE} # --max-tokens 512 --distributed-port 23460 --distributed-world-size ${DIST_SIZE}
