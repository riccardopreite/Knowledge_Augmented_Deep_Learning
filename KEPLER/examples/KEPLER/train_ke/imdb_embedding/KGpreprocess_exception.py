...
def genSample(triples, args, split, Qdesc, true_head, true_tail):
    ...
    sizes=[]
    nE=len(Qdesc)
    for h, r, t in tqdm(triples, desc = 'Gen sample Progress Bar'):
        try:
            rel.append(r)
            fHead.write(Qdesc[h])
            fTail.write(Qdesc[t])
    ...
    fnHead.close()
    fnTail.close()
    np.save(os.path.join("exception_",args.dumpPath, "relation", split)+".npy", np.array(rel))
    np.save(os.path.join("exception_",args.dumpPath, "sizes", split)+".npy", np.array(sizes))
